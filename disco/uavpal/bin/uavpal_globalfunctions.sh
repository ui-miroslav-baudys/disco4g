...
#       callers invoking this function using method "get" should handle (echoed) return values using var=$(hilink_api)
	...
	if [ -f /tmp/hilink_login_required ]; then
		sessionInfoLogin=$(/data/ftp/uavpal/bin/curl -s -X POST "http://${hilink_router_ip}/api/user/login" -d "<request><Username>admin</Username><Password>$(printf "%s" "admin" |base64)</Password><password_type>3</password_type></request>" -H "Cookie: $cookie" -H "__RequestVerificationToken: $token" --dump-header - 2>/dev/null)
		if printf "%s" "$sessionInfoLogin" | grep '<code>108006\|<code>108007' ; then
		...
		cookie=$(printf "%s" "$sessionInfoLogin" | grep "SessionID=" | cut -d ':' -f2 | cut -d ';' -f1)
		token=$(printf "%s" "$sessionInfoLogin" | grep "__RequestVerificationTokenone" | cut -d ':' -f2)
...
firewall()
{
	...
	for i in $ip_block; do 
		iptables -I INPUT -p tcp -i "${1}" --dport "$i" -j DROP; 
	done
}
...
send_message()
{
	...
	if ! check_connection_repeated; then
...
connect_hilink()
{
	...
	hilink_router_ip=$(printf "%s.1" "$(echo "$hilink_ip" | cut -d '.' -f 1,2,3)")
...
connect_stick()
{
	...
	# shellcheck disable=SC2154
	/data/ftp/uavpal/bin/pppd "${serial_ppp_dev}" \
...
check_connection()
{
	...
	ping_destinations="1.1.1.1 8.8.8.8 192.5.5.241 199.7.83.42" # cloudflare, google-public-dns-a.google.com, f.root-servers.org, l.root-servers.org
	for check in $ping_destinations; do
		for i in $(seq 1 $ping_retries_per_destination); do
			if ping -W 5 -c 1 "$check" >/dev/null 2>&1; then
				return 0
			fi
			sleep 1
		done
