...
change_led_color()
{
	case "$1" in
	off)
		printf "" > /dev/input/event1
	;;
	red)
		printf "" > /dev/input/event1
	;;
	green)
		printf "" > /dev/input/event1
	;;
	blue)
		printf "" > /dev/input/event1
	;;
	magenta)
		printf "" > /dev/input/event1
	;;
	white)
		printf "" > /dev/input/event1
	;;
	*)
	esac
}
...
power_btn_listener()
{
	if [ "$pow_pid" -eq "0" ]; then
		...
		if [ "$platform" = "mpp" ]; then
			(evtest /dev/input/event0 | grep -m 1 -e "type 1 (EV_KEY), code 295 (BTN_BASE2), value 1" | while read -r; do
			...
			(evtest /dev/input/event0 | grep -m 1 -e "type 1 (EV_KEY), code 116 (KEY_POWER), value 1" | while read -r; do
...
main()
{
	...
	until [ "$(ulogcat -d |grep mppd |grep -c "state=IDLE")" -gt "0" ]; do sleep 1; done
	...
	while true; do
		...
		while true; do
			evtest ${input_dev_settings} | grep -m 1 -e "type 1 (EV_KEY), code 288 (BTN_TRIGGER), value 1" | while read -r; do
...
switch_to_lte()
{
	...
	for p in $(seq 1 $wifi_connection_attempts); do
		...
		if [ "$platform" = "mpp" ]; then
			wifid-cli --suffix uavpal connect "$(conf_read ssid)" 0 "$(conf_read wpa)"
		else
			wifid-cli connect "$(conf_read ssid)" 0 "$(conf_read wpa)"
		fi

		for _ in $(seq 1 $wifi_connection_timeout_seconds); do
			if [ "$platform" = "mpp" ]; then
				wifi_connection_status=$(wifid-cli --suffix uavpal status 2>&1 |grep state |awk '{print $3}')
			else
				wifi_connection_status=$(wifid-cli status 2>&1 |grep state |awk '{print $3}')
			fi
	...
	do
		wifi_dhcp_ip=$(ifconfig wlan0 |grep inet |awk '{print $2}' |grep -c -v '192.168.42')
	...
	for p in $(seq 1 $zerotier_iface_timeout_seconds); do
		...
		if [ "$platform" = "mpp2" ]; then
			...
			if [ "$(ifconfig "$zt_interface" |grep -c "$zt_ip_addr")" -gt "0" ] ; then
